...
import { initializeSocket, sendMessage, setMessages, addMessage } from '../redux/actions/messageActions';
...
const TeacherMess = () => {
  ...
  useEffect(() => {
    if (socket) {
      socket.on("loadMessages", (loadedMessages) => {
        const roomId = getRoomId(currentUserId, selectedUser.id);
        dispatch(setMessages(roomId, loadedMessages));
      });
    }
  ...
  }, [dispatch, currentUserId, selectedUser]);
